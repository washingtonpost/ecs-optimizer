class ServiceOptimizer(object):
    ...
    def optimize(self, verbose, cluster, service, start_date, end_date, cpu_over_reserve, cpu_under_reserve, mem_over_reserve):
        ...
        new_mem_limit = self.recommend_memory(mem_utilization, mem_hard_limit, mem_soft_limit, mem_over_reserve)
    ...
    def recommend_memory(self, utilization, old_hard_limit, old_soft_limit, over_reserve):
        memory = old_soft_limit*(utilization/100.0)
        new_limit = self.calc_memory_limit(memory * (1.0 + over_reserve))

        return self.optimal_limit(memory, old_soft_limit, new_limit, over_reserve)
